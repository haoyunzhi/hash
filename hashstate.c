...
HashReturn Init(hashState *state, int hashbitlen)
{
    switch(hashbitlen) {
        case 0: // Default parameters, arbitrary length output
            ...
            break;
        case 256:
            InitSponge((spongeState*)state, 1408, 512);
            break;
        case 384:
            InitSponge((spongeState*)state, 1152, 768);
            break;
        case 512:
            InitSponge((spongeState*)state, 896, 1024);
            break;
        default:
            return BAD_HASHLEN;
    }
...
HashReturn Hash(int hashbitlen, const BitSequence *data, DataLength databitlen, BitSequence *hashval)
{
    ...
    if ( (hashbitlen != 256) && (hashbitlen != 384) && (hashbitlen != 512))
